...
//!
//! ## Security Features:
//! - **Nostr Signature Verification**: CSFS verifies Schnorr signatures from Nostr events
//! - **Hardcoded Constraints**: Expected signature and pubkey are embedded in script
//! - **Single Script Path**: Simplified design with one spending condition
...
use bitcoin::{
    ...
    transaction::Version,
    Address, Amount, Network, OutPoint, ScriptBuf, Sequence, Transaction, TxIn,
    TxOut, Witness,
};
use nostr::{EventBuilder, Keys, Kind, Event, JsonUtil};
...
impl NostrVault {
    ...
    pub fn new(amount: u64) -> Result<Self> {
        ...
        if nostr_pubkey_bytes.len() != 32 {
            return Err(anyhow!("Nostr pubkey must be 32 bytes for CSFS compatibility"));
        }
