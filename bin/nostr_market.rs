...
//! ```bash
//! # Create a new market
//! nostr_market create --question "Bitcoin above $100k by EOY 2024?" \
...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Status { market_id } => {
            show_market_status(&storage, market_id).await
        }
        
        Commands::List => {
            list_markets(&storage).await
        }
        ...
        Commands::Demo { scenario } => {
            run_demo(&storage, &scenario).await
        }
    }
}
...
async fn place_bet(
    ...
    let funding_txid = format!("demo_tx_{}_{}_{}", market.market_id, outcome.to_ascii_uppercase(), amount);
    ...
    println!("   🅰️  Outcome A: {} sats ({:.1}x odds)", market.get_total_a(), market.get_odds_a());
    println!("   🅱️  Outcome B: {} sats ({:.1}x odds)", market.get_total_b(), market.get_odds_b());
...
async fn show_market_status(storage: &MarketStorage, market_id: String) -> Result<()> {
    ...
    println!("   🅰️  Outcome A: {} sats ({} bets, {:.1}x odds)", 
             market.get_total_a(), market.bets_a.len(), market.get_odds_a());
    println!("   🅱️  Outcome B: {} sats ({} bets, {:.1}x odds)", 
             market.get_total_b(), market.bets_b.len(), market.get_odds_b());
    println!("   💰 Total Pool: {} sats", market.total_amount);
    println!();
    ...
    if market.settled {
        ...
        println!("🎲 Market is active - accepting bets until {}", 
...
async fn list_markets(storage: &MarketStorage) -> Result<()> {
    ...
    for market_id in market_ids {
        if let Ok(market) = storage.load_market(&market_id) {
            ...
            println!("   📊 {} | Total: {} sats", market.get_status(), market.total_amount);
            println!("   ⏰ Settlement: {}", format_timestamp(market.settlement_timestamp));
            println!();
        }
    }
...
async fn run_basic_demo(storage: &MarketStorage) -> Result<()> {
    ...
    let settlement_time = SystemTime::now()
        ...
        .as_secs() + 3600; // Settlement in 1 hour
        
    let oracle_pubkey = "02".to_string() + &"a".repeat(62); // Demo oracle pubkey
    
    let market = NostrPredictionMarket::new(
        "Will Bitcoin be above $100k by end of 2024?".to_string(),
    ...
    println!("   🅰️  Alice: 50,000 sats on 'Yes' ({:.1}x odds)", demo_market.get_odds_a());
    println!("   🅱️  Bob: 30,000 sats on 'No' ({:.1}x odds)", demo_market.get_odds_b());
    ...
    println!("   ⏰ Settlement time: {}", format_timestamp(settlement_time));
    ...
    println!("💡 Try:");
    println!("   nostr_market status --market-id {}", demo_market.market_id);
